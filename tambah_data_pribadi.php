<?php
    // menghubungkan file functions
    require "functions_pribadi.php";

    $conn =  mysqli_connect("localhost", "root", "", "universitas_x");

    if( isset($_POST['submit']) ) {
        // cek apakah data berhasil ditambahkan
        // data dari elemen form diambil
        // dimasukkan ke dalam tambah() 
        if( tambah($_POST) > 0 ) {
            echo "
                <script>
                    alert('data berhasil ditambahkan!');
                    document.location.href = 'data_mahasiswa_pribadi.php';
                </script>
            ";
            
        }
        else {
            echo "
                <script>
                    alert('data gagal ditambahkan!');
                    // document.location.href = 'data_mahasiswa_pribadi.php';
                </script>
            ";
            // echo mysqli_error($conn);
            // die;
        }
    }


?>

<!DOCTYPE html>
<html lang="en">
<head>
    <title>Tambah Data Pribadi</title>

    <!-- link style css -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-eOJMYsd53ii+scO/bJGFsiCZc+5NDVN2yr8+0RDqr0Ql0h+rP48ckxlpbzKgwra6" crossorigin="anonymous">

</head>
<body>
    
    <div class="container">
        <div class="mt-5">
        <div class="row">
            <div class="col-md-12">
                <h1 class="text-center">Tambah Data Pribadi Mahasiswa</h1>
                <hr>
            </div>
        </div>

        <div class="row">
            <div class="mt-5">
            <div class="col-md-10">
                <div class="card shadow-lg border-0">
                    <div class="card-body">
                        <h3>Form Tambah Data Pribadi</h3><hr>
                        <form action="" method="POST">
                            <div class="form-group">
                                <!-- form untuk input data -->
                                <label for="nama">Nama Mahasiswa</label>
                                <input type="text" class="form-control" name="nama" id="nama" placeholder="Input nama ">
                                <br>
                                <label for="ttl">Tempat, tanggal lahir</label>
                                <input type="text" class="form-control" name="ttl" id="ttl" placeholder="Input tempat, tanggal lahir ">
                                <br>
                                <label for="alamat">Alamat</label>
                                <input type="text" class="form-control" name="alamat" id="alamat" placeholder="Input alamat ">
                                <br>
                                <label for="agama">Agama</label>
                                <input type="text" class="form-control" name="agama" id="agama" placeholder="Input agama ">
                                <br>
                                <label for="golongan_darah">Golongan darah</label>
                                <input type="text" class="form-control" name="golongan_darah" id="golongan_darah" placeholder="Input kelas ">
                                <br>
                                <label for="jalur_masuk">Jalur Masuk</label>
                                <input type="text" class="form-control" name="jalur_masuk" id="jalur_masuk" placeholder="Input jalur_masuk ">
                            </div>
                            <br>
                            
                            <div class="form-group">
                                <div class="float-sm-end">
                                <button type="submit" class="btn btn-primary btn-block" name="submit">Tambahkan Data Mahasiswa</button>
                                </div>
                            </div>
                            <br>
                        </form>
                    </div>
                </div>
            </div>
        </div>
        </div>
        </div>
    </div>

</body>
</html>